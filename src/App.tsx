...
export default function App() {
  ...
  const activeEnzyme = enzymes[selectedKey] || DEFAULT_ENZYMES['Hexokinase'];
      ...
      if (isDenatured) {
        ...
        if (renderStyle === 'vdw') {
          ...
          viewer.addSurface(
            ...
            { opacity: 0.65, colorscheme: { prop: 'resi', gradient: 'roygb' } },
            { hetflag: false }
          );
        } else if (renderStyle === 'spheres') {
          viewer.setStyle(
            { hetflag: false },
            { sphere: { colorscheme: { prop: 'resi', gradient: 'roygb' }, scale: 0.9 } }
          );
        } else {
          viewer.setStyle({ hetflag: false }, { cartoon: { color: 'spectrum' } });
        }
  ...
  // Cleanup instances on viewport change
  useEffect(() => {
    if (viewMode === 'open') {
      closedViewerInstance.current = null;
    } else if (viewMode === 'closed') {
      openViewerInstance.current = null;
    }
  }, [viewMode]);
